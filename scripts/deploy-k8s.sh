...
while (($#)); do
  case "$1" in
    --context|--profile)
      ...
      if [[ "$1" == --context ]]; then context="$2"; else profile="$2"; fi
      shift 2 ;;
    -h|--help) echo 'Usage: scripts/deploy-k8s.sh [--context NAME] [--profile local|multi-node]'; exit 0 ;;
    *) echo "Unknown argument: $1" >&2; exit 2 ;;
  esac
done
case "$profile" in local) manifests=kubernetes-local ;; multi-node) manifests=kubernetes ;; *) echo 'Profile must be local or multi-node' >&2; exit 2 ;; esac
...
required=3 if sys.argv[1]=="multi-node" else 1
if len(ready)<required: raise SystemExit(f"{sys.argv[1]} profile needs at least {required} Ready, schedulable nodes; found {len(ready)}. Use --profile local on OrbStack.")
' "$profile"
...
'
echo "Deploying HomeHub: context=$context profile=$profile"
...
k apply -k "$manifests"
# The CNPG operator reconciles existing Pod affinity after switching profiles.
# No Pod/PVC/Secret deletion is required to repair the accidental base deployment.
