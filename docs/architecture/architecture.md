...
The default `kubernetes/` deployment requires database instances on different nodes. It needs at least three schedulable nodes, suitable independent persistent storage, and an available Kubernetes control plane. `kubernetes-local/` relaxes anti-affinity for a one-node classroom demonstration. Three instances on the same laptop do not protect against loss of that laptop, its storage, or its control plane.
